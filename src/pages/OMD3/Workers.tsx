...
export const Workers = () => {
    ...
    const [workers, setWorkers] = useState<Worker[]>([
        { id: 'WRK-OMD3-01', name: 'Storage-Node-A1', description: 'DePIN distributed storage node.', status: 'Active' },
        { id: 'WRK-OMD3-02', name: 'Replication-Svc', description: 'Data replication across regional shards.', status: 'Active' },
        { id: 'WRK-OMD3-03', name: 'Bandwidth-Relay', description: 'High-speed relay for content delivery.', status: 'Stopped' },
        { id: 'WRK-OMD3-04', name: 'Proof-Of-Physical', description: 'Verifies physical infrastructure uptime.', status: 'Active' },
        { id: 'WRK-OMD3-05', name: 'Gateway-Router', description: 'Ingress controller for OMD3 network.', status: 'Active' },
    ]);
    ...
    const [history, setHistory] = useState<{ cpu: number[], mem: number[] }>({
        cpu: Array(40).fill(45),
        mem: Array(40).fill(78)
    });
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-4">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-bold text-foreground">Workers OMD3 (DePIN)</h1>
                        <p className="text-muted-foreground text-sm font-medium">Gestión de infraestructura física descentralizada.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card className="shadow-lg border-t-4 border-t-purple-500">
                        ...
                        <CardContent className="pt-2">
                            <div className="flex items-center justify-between mb-1">
                                ...
                                <span className="text-sm font-black text-purple-500">{Math.round(cpuLoad)}%</span>
                            </div>
                            <div className="w-full h-2 bg-muted rounded-full mb-3">
                                <div className="h-full bg-purple-500 rounded-full transition-all duration-500" style={{ width: `${cpuLoad}%` }} />
                            </div>
                    ...
                    <Card className="shadow-lg border-t-4 border-t-emerald-500">
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-lg font-bold">Active Nodes</CardTitle>
                            <CheckCircle2 className="h-5 w-5 text-emerald-500" />
                        </CardHeader>
                        <CardContent className="pt-2">
                            <div className="flex items-end gap-2 mb-2">
                                <div className="text-3xl font-black text-foreground">{activeCount}</div>
                                <div className="text-sm text-muted-foreground font-medium mb-1">Online Devices</div>
                            </div>
                    ...
                    <Card className="shadow-lg border-t-4 border-t-red-500">
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-lg font-bold">Inactive nodes</CardTitle>
                            <XCircle className="h-5 w-5 text-red-500" />
                        </CardHeader>
                        <CardContent className="pt-2">
                            <div className="flex items-end gap-2 mb-2">
                                <div className="text-3xl font-black text-foreground">{inactiveCount}</div>
                                <div className="text-sm text-muted-foreground font-medium mb-1">Offline Devices</div>
                            </div>
                            <div className="w-full h-2 bg-muted rounded-full">
                ...
                <Card className="shadow-lg overflow-hidden">
                    <CardHeader className="border-b border-border bg-muted/10 pb-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <CardTitle className="text-xl font-bold">Estado de Nodos</CardTitle>
                                <CardDescription>Registro completo de infraestructura física.</CardDescription>
                            </div>
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-muted/30">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider">Función</th>
                ...
                <Card className="shadow-lg">
                    <CardHeader className="flex flex-row items-center justify-between pb-4 px-8">
                        <div>
                            <CardTitle className="text-xl font-bold">Network Performance</CardTitle>
                            <CardDescription className="text-xs font-medium uppercase tracking-widest">Aggregate Throughput Metrics</CardDescription>
                        </div>
                        <div className="flex gap-4 text-xs font-bold">
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full bg-primary ring-2 ring-primary/20"></div>
                                <span>Bandwidth ({Math.round(cpuLoad)}%)</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full bg-purple-500 ring-2 ring-purple-500/20"></div>
                                <span>Storage ({Math.round(memLoad)}%)</span>
                            </div>
                        </div>
                    </CardHeader>
