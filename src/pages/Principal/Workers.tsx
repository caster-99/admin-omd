...
export const Workers = () => {
    ...
    const [workers, setWorkers] = useState<Worker[]>([
        { id: 'WRK-8821', name: 'Data-Miner-01', description: 'Primary data extraction unit for external API a...', status: 'Active' },
        { id: 'WRK-9932', name: 'Image-Proc-Alpha', description: 'Batch processing for image resizing and optim...', status: 'Active' },
        { id: 'WRK-1102', name: 'Email-Notifier-V2', description: 'Scheduled email dispatch worker.', status: 'Stopped' },
        { id: 'WRK-4451', name: 'Backup-Daemon', description: 'Continuous database backup service.', status: 'Active' },
        { id: 'WRK-7721', name: 'Analytics-Engine', description: 'Real-time event processing for user dashboar...', status: 'Active' },
    ]);
    ...
    // --- Mock API Handlers (Prepared for Axios) ---
    ...
    // Fetch Workers (Placeholder for Axios)
    ...
    // Toggle Worker Status (Functional Switch)
    const handleToggleStatus = async (workerId: string) => {
        try {
            ...
            const newStatus = worker.status === 'Active' ? 'Stopped' : 'Active';

            // --- Axios Integration Point ---
            // await axios.patch(`/api/workers/${workerId}`, { status: newStatus });
            ...
            console.log(`Worker ${workerId} status changed to ${newStatus}`);
        } catch (error) {
            console.error('Error toggling worker status:', error);
            // Rollback on error if using real API
        }
    };
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-4">
                    <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground text-sm font-medium">Telemetry-driven management plane for distributed workers.</p>
                    </div>
                ...
                <Card className="shadow-lg overflow-hidden">
                    <CardHeader className="border-b border-border bg-muted/10 pb-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <CardTitle className="text-xl font-bold">{t('workers.detailedStatus')}</CardTitle>
                                <CardDescription>Comprehensive registry of individual service instances.</CardDescription>
                            </div>
                    ...
                    <CardContent className="p-0 overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-muted/30">
                                <tr>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider">{t('workers.table.name')}</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider">{t('workers.table.description')}</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider">{t('workers.table.status')}</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-right">{t('workers.table.control')}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border">
                                {filteredWorkers.map((worker) => (
                                    <tr key={worker.id} className="hover:bg-muted/10 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <Chip
                                                label={t(`workers.status.${worker.status.toLowerCase()}`)}
                ...
                <Card className="shadow-lg">
                    <CardHeader className="flex flex-row items-center justify-between pb-4 px-8">
                        <div>
                            <CardTitle className="text-xl font-bold">{t('workers.chart.title')}</CardTitle>
                        ...
                        <div className="flex gap-4 text-xs font-bold">
                            <div className="flex items-center gap-2">
                                ...
                                <span>{t('workers.chart.cpu')} ({Math.round(cpuLoad)}%)</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full bg-purple-500 ring-2 ring-purple-500/20"></div>
                                <span>{t('workers.chart.memory')} ({Math.round(memLoad)}%)</span>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="h-72 pt-4 pb-8 relative px-8 overflow-hidden">
                        ...
                        <div className="flex justify-between mt-4 text-[9px] text-muted-foreground font-black border-t border-border pt-4 px-2 uppercase tracking-tighter">
                            <span>-30s</span>
                            <span>-20s</span>
                            <span>-10s</span>
