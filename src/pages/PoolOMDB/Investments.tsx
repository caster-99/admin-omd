...
const MOCK_INVESTMENTS = Array.from({ length: 24 }, (_, i) => {
    const types = [
        { name: 'OMDB Staking', icon: <Coins className="h-4 w-4" />, method: 'Internal Wallet' },
        { name: 'Gov Bond', icon: <Globe className="h-4 w-4" />, method: 'Direct Deposit' },
        { name: 'Liquidity Pool', icon: <Bitcoin className="h-4 w-4" />, method: 'Smart Contract' },
        { name: 'Yield Farm', icon: <BarChart3 className="h-4 w-4" />, method: 'DeFi Protocol' }
    ];
    ...
    return {
        id: `#INV-OMDB-${10542 - i}`,
        asset: type.name,
        type: i % 3 === 0 ? 'Staking' : i % 3 === 1 ? 'Bond' : 'Liquidity',
        icon: type.icon,
        method: type.method,
        user: {
            name: i % 2 === 0 ? 'Maria Gonzales' : i % 3 === 0 ? 'John Doe' : 'Jane Smith',
            ...
            <div className="flex flex-col gap-6 py-6 text-foreground w-full">
                ...
                <div>
                    <div className="flex items-center justify-between">
                        <div>
                            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Inversiones Pool OMDB</h2>
